...
| # | Aufgabe | Hängt ab von | Details |
|---|---|---|---|
| 6.1 | **hpak-Packaging**: HpakWriter + KeyDerivation implementieren, asset_compiler → Packer-Kette, GameApplication lädt aus .hpak | 1.4 | ✅ Forts. 55 — HpakWriter/HpakReader in HorizonCore; .hpak-Format (magic+TOC+Datenblöcke, 36B/Entry); KeyDerivation PBKDF2-HMAC-SHA256 (standalone, kein mbedtls); optionale XOR-Verschlüsselung; HAsset::toBytes/openData; ContentManager::loadPak+loadAssetFromMemory; hpak_packer-CLI (--secret); **LZ4-Kompression verdrahtet (Forts. 57): PackSettings.compress, compress→encrypt (write)/decrypt→decompress (read), system-lz4 via pkg-config+find_path, graceful fallback; 4 neue LZ4-Tests; 379 Tests grün** — **hpak v2-Überarbeitung (Forts. 62, 02.07.2026, uncommitted): Format v2 (64B-FileHeader + 56B-EntryDesc mit `codec`/`contentHash`/`nonce`/`buildId`/`baseArchiveId`, TOC nach UUID sortiert→Binärsuche, `tocHash`+`contentHash` FNV-1a-Integrität, persistenter File-Handle statt fd-Reopen); LZ4→LZ4HC-9 + zstd -19 (System-libs, in CMake gespiegelt); XOR→AES-256-GCM via OpenSSL EVP (`Aes256Gcm`, per-Entry-Random-Nonce+Auth-Tag), Random-Key in `project.hcfg`, GameApplication übergibt Key an `loadPak` (**D1-Fix**: verschlüsselte Paks waren vorher unladbar); `loadAssetFromMemory` dedupliziert→delegiert an `parseAndRegisterAsset` (**Bugfix**: SkeletalMesh/Scene/Script/Font/Shader wurden aus jedem Pak still verworfen); adversarielle Multi-Agent-Review→2 Bugs gefixt (failbit-Poisoning kritisch + int-Overflow-Guard). Plan: `docs/hpak-format-plan.md`. 501 Tests grün** |
...
| 6.3 | **„Build Game"-Pipeline im Editor**: Standalone-Export (Executable + .hpak) pro Plattform | 6.1 | ✅ Forts. 58+59 — ProjectConfig+ProjectConfigLoader in HorizonCore; ProjectExporter packt .hasset→.hpak+schreibt project.hcfg+kopiert .hescene+kopiert Binary/dylibs (ExportSettings.gameRuntimeDir); ExportResult.binaryFilesCopied; GameApplication::OnInit lädt project.hcfg + ContentManager::loadPak; Editor: Build>Export-Modal; HorizonGame BUILD_RPATH @executable_path (macOS); 12 Tests (4 neu für Binary-Copy); 399 Tests grün — **Export-Profile + Async + Excludes (Forts. 64, 03.07.2026, fcf0644):** benannte `ExportProfile` (Codec/Encrypt/Mods/Startup-Szene/Output-Dir/Exclude-Globs) persistiert im `.heproj` (Read-Modify-Write, unbekannte Keys+startupScene überleben; Legacy-Manifeste seeden Development+Shipping); Export-Dialog mit Profil-Dropdown/Save/Save-As + Szenen-Auswahl + Excludes-Multiline; Export läuft async auf Worker-Thread mit ProgressBar (HpakWriter-Progress-Callback, join bei Completion+Shutdown); `Hpak::globMatch` (`*` spannt `/`) in addDirectory + `hpak_packer --exclude`; Default-Output-Dir-Bug gefixt (lag in `X.heproj/Export`); +10 Review-Fixes (Worker-try/catch, Modal-Reopen-Guard, non-throwing fs-Iteration, atomarer .heproj-Save, HAsset-Size-Overflow-Guard) — **Inkrementelles Packen + Plattform-Ziele (Forts. 65, 03.07.2026, 876076e):** Reuse unveränderter Assets byte-verbatim aus dem Vor-Pak (Cache-Key = hash64 des REWRITTEN Blobs → Referenz-Umbenennungen invalidieren Dependents; `.hpak.manifest`-Sidecar gated auf pakTocHash+Settings-Fingerprint; `HpakReader::readStoredEntry`/`HpakWriter::addPackedEntry`; verschlüsselte Exporte reusen den Key aus project.hcfg); `ExportPlatform` Host/Windows/macOS/Linux pro Profil (Runtime-Bundle aus `../GameRuntimes/<Name>/`, Output in Plattform-Unterordner, Pre-Flight-Check) — **Lauffähige Exporte + Embedded Key (Forts. 66, 03.07.2026, 18fadc7):** Exporte waren still daten-only (`../Game` existiert nur im Deploy-Layout, CLion-Build-Tree → 0 Binaries als „OK"); jetzt `findRuntimeBundle` (Aufwärts-Walk, akzeptiert nur Verzeichnisse MIT HorizonGame[.exe]), harter Pre-Flight auch für Host, Exporter-Fehler bei 0 kopierten Dateien, `add_dependencies(HorizonEditor HorizonGame)`; **Encryption-Key wird in die Game-Executable gepatcht** (64B-Block `EmbeddedPakKey.h` magic|hasKey|pad|key, Magic im Exporter aus Teilen assembliert gegen False-Hits, alle Vorkommen gepatcht/Universal-Binaries, macOS ad-hoc-Re-Sign via codesign — auf HW verifiziert: gepatchtes Binary startet), project.hcfg trägt dann nur noch das Encrypted-Flag; Runtime bevorzugt Embedded-Key, hcfg-Key = Legacy-Fallback; Incremental-Key-Recovery aus dem gepatchten Binary (Null-Key nie als Reuse-Quelle) + kombinierte Review 15 Findings/7 Root-Causes gefixt (silent-failures→harte Fehler) — **macOS .app-Bundle (Forts. 67, 06e9e4b):** Export-Option „macOS .app bundle" (nur macOS-Host, Host/macOS-Target); binDir/dataDir-Split: Executable+Engine-dylibs→Contents/MacOS (`@executable_path`), pak/hcfg/GameLogic/Szene→Contents/Resources (= `SDL_GetBasePath()` im Bundle, Runtime unverändert), generierte Info.plist, ganzes Bundle ad-hoc `codesign --deep` (Sign-Fehler = harter Fehler); Key-Patch weiter in die gebundelte Executable; adversarielle Review 0 Findings (4 Linsen sauber); real-HW verifiziert (voll-signiertes Bundle mit echten dylibs startet ohne SIGKILL); 559 Tests grün |
...
- **GPU-Upload-Budgeting pro Frame** (Renderer, Streaming-Politur) — 🔴 offen. Beim Asset-Streaming (6.1/6.4) lädt der Renderer Meshes/Texturen beim *ersten Sehen* synchron zur GPU hoch („upload on first sight"); ein großer Pop-in-Schub kann einen Frame rucken. Fix: die GPU-Upload-Menge pro Frame deckeln (Upload-Queue, analog zum bereits budgetierten `pollAsyncResults(16)` auf CPU-Seite). Backend-lokal, GL+Metal zuerst. Das ist der *letzte* Main-Thread-Kostenpunkt beim Streaming außerhalb von hpak — die CPU-Seite (I/O/Decompress/Decrypt off-thread, Parse/Register budgetiert) ist bereits ruckelfrei.
