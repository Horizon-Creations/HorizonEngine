...
public:
    ...
    // Honoured by every backend that renders cascades (OpenGL, Metal, D3D11, D3D12, Vulkan): distance /
    ...
    // Debug: tint each lit fragment by its shadow cascade index (every CSM
    // backend: Metal, OpenGL, D3D11, D3D12, Vulkan) so the cascade split
    // placement can be verified visually. No-op on the software renderer.
