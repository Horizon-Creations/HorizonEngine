...
struct CompiledVarInfo
{
    ...
    // Mirrors Variable::replicated / repNotify (docs/gameplay-replication-plan.md
    // §6.1). DEFAULTED, which is what lets every generated table written before
    // replication existed keep compiling: the four-element aggregate initialiser
    // the codegen has always emitted still fills the members it names and leaves
    // these two at "this variable does not replicate" — the truth for a class
    // generated by an older build.
    bool        replicated = false;
    bool        repNotify  = false;
    // Mirrors Variable::saveGame. Appended last and defaulted for the same
