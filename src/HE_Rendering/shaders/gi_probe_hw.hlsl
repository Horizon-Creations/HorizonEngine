...
// The DDGI probe-update kernel exists once per API dialect and once per
// traversal path. Nothing generates them from a common source:
//
//   GLSL SW  src/HE_Rendering/shaders/gi_probe.comp      (Vulkan, CPU BVH)
//   GLSL HW  src/HE_Rendering/shaders/gi_probe_hw.comp   (Vulkan, ray query)
//   HLSL SW  src/HE_Rendering/src/Backends/D3D_Shared/HlslSources.h
//            `kGiTraversalHLSL` + `kGiProbeCSHLSL`       (D3D11 and D3D12)
//   HLSL HW  src/HE_Rendering/shaders/gi_probe_hw.hlsl   (this file)
//   GLSL SW  src/HE_Rendering/src/Backends/OpenGL/OpenGLRenderer.cpp
//            `kGiTraversalGLSL` + `kGiProbeCS`           (GL 4.3)
//   MSL  SW  src/HE_Rendering/src/Backends/Metal/MetalRenderer.mm `kGISWMSL`
//   MSL  HW  src/HE_Rendering/src/Backends/Metal/MetalRenderer.mm `kGIProbeMSL`
//
// WHAT MUST NOT DRIFT, inside this family: kOctSize and the octDecode mapping
// (they define the atlas layout), the secondary-ray parameters (hitNormal * 0.05
// bias, tMin/tMax 0.02 / 10000, the 0.1 slack on the local ray), the adaptive
// hysteresis constants, and the 8x8 thread group (one thread per octahedral
// texel — the host dispatches on that assumption).
//
// AND ACROSS FAMILIES: kOctSize is also baked into every SAMPLER of the atlas —
// scene.frag's `GI_PROBE_OCT` / giOctEncode, the same block in kUnlitFS
...
// simply converge to different irradiance on the affected backend/path —
// too-fast hysteresis flickers, a wrong bias leaks light through walls, a wrong
// tile size samples neighbouring probes. Nothing here is covered by an image
// test.
//
...
// in files of their own (and kOctSize against the samplers it can reach). The
// four embedded as C++ string literals are out of its reach — change one, change
// all seven, and run that test.
