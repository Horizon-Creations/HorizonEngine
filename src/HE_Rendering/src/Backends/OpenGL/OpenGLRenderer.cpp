...
// Erzeugt aus shaders/sky_core.glsl (cmake/embed_glsl.cmake). Der analytische
// Himmel liegt genau dort und wird von Vulkan aus DERSELBEN Datei per #include
// gezogen; dieser Header ist nur die C++-Verpackung, weil der GL-Shader ein
// Stringliteral braucht, das zur Laufzeit an //#SKYFUNC# gespleisst wird.
#include "sky_core_glsl.h"
#include "sky_aurora_glsl.h"   // dieselbe Mechanik fuer die 3D-Aurora
...
void main()
{
	...
	if (nightF > 0.0)
	{
		...
		col += applyAurora3D(dir, uCameraPos, uTime, uAurora, uAuroraColor, uAuroraColorTop, gl_FragCoord.xy);
...
//
// Der Rumpf stand bis hierher als 173-Zeilen-Literal an dieser Stelle und ist
// nach shaders/sky_core.glsl umgezogen — unveraendert, Zeichen fuer Zeichen.
// Grund: derselbe Kern lag zusaetzlich als Handspiegel in HlslSources.h und in
// shaders/sky.frag, und beide Kopien waren beim alten Gradienten stehen
// geblieben, waehrend hier laengst die Streuungsintegrale liefen. Jetzt gibt es
// den Text einmal; GL bekommt ihn als generiertes Literal, Vulkan per #include.
static const char* kSkyFuncGLSL = kSkyCoreGLSL;
...
static std::string injectSkyFunc(const char* src)
{
	...
	// Zweiter Marker, gleiche Mechanik: die 3D-Aurora liegt in
	// shaders/sky_aurora.glsl, weil Vulkan und D3D sie aus derselben Datei
	// beziehen. Sie sitzt an skyIgns alter Stelle -- also VOR allem, was sie
	// braucht (starHash, cloudNoise) und vor allem, was skyIgn benutzt.
	const std::string aurora = "//#SKYAURORA#";
	if (size_t pos = s.find(aurora); pos != std::string::npos)
		s.replace(pos, aurora.size(), kSkyAuroraGLSL);
	return s;
}
