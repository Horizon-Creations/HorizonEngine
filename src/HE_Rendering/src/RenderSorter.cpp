                                      ...
                                      bool                          sectionAware)
{
	...
	for (const DrawCall& dc : drawCalls)
	{
		...
		// indexCount; of the backends collecting through here D3D11 does too
		// (sectionAware), D3D12 and Vulkan still draw the whole index buffer per
		// DrawCall. For those a multi-section mesh's second, third… draw would
		// repaint the entire mesh in another material, so they get slot 0 alone —
