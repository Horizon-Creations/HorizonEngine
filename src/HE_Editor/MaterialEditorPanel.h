...
namespace MaterialEditorPanel
{
	...
	// The same two questions, addressed CONTENT-RELATIVELY — what the MCP
	// material tools hold an asset by (see McpMaterialHooks). Both walk on the
	// state's stored relPath rather than string-matching two spellings of an
	// absolute path, and both answer as if the panel never held the asset when it
	// does not. Same shape and same reason as InputAssetPanel's pair.
