...
// The editor's own settings (EditorConfig) and its mode enum. Moved into a
// header of their own so the settings table the MCP tools drive can be
// compiled — and tested — without this file's ImGui/SDL/renderer stack.
...
private:
	...
	// Write m_editorConfig into config.json. The ONE place that knows the config
	// key for each field — it used to be a block at the end of OnShutdown, which
	// meant a setting changed through `settings_set` only reached disk if the
	// editor was closed cleanly afterwards. OnShutdown still calls it; so does
	// the settings tool, which then reports `persisted: true` truthfully.
	void writeEditorConfig();
