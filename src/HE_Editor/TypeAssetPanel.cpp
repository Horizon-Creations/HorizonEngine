...
// ── Addressed content-relatively (the MCP type tools) ───────────────────────
...
// same shape InputAssetPanel::stateByContentPath has and for the same reason.
...
bool TypeAssetPanel::isDirtyByContentPath(const std::string& contentPath)
{
	const PanelState* st = stateByContentPath(contentPath);
	return st && st->dirty;
}

bool TypeAssetPanel::reloadByContentPath(const std::string& contentPath)
{
	PanelState* st = stateByContentPath(contentPath);
	if (!st) return false;
	// The same two lines reloadFromDisk sets, and safe for the same reason: the
	// MCP tools refuse outright when this tab has unsaved edits (McpTypeHooks),
