...
std::string fieldCppType(const StructField& f, const NameTable& nt)
{
    ...
    switch (f.kind())
    {
    // A Set is a vector because its ITERATION ORDER is its insertion order
    // (see ContainerKind) — an std::set would sort it and stop matching what
    // the interpreter and the generated HorizonCode C++ do. Same reason a Map
    // is a vector of pairs rather than an std::map or unordered_map. Elements
    // are unique; this header describes the data, it does not police it.
    case HorizonCode::ContainerKind::Set:
        return "std::vector<" + base + "> /* unique, in insertion order */";
    case HorizonCode::ContainerKind::Map:
    {
        std::string key;
        switch (f.keyType)
        {
        case PinType::Int:    key = "int"; break;
        case PinType::String: key = "std::string"; break;
        case PinType::Ref:    key = "unsigned int"; break;
        case PinType::Enum:   key = nt.of(f.keyTypeName); break;
        default:              key = "int"; break;
        }
        return "std::vector<std::pair<" + key + ", " + base +
                             ...
                             const TypeRegistry& reg)
{
    if (f.isArray)
    {
        // One authored slot as a bare expression. The scalar view clears BOTH
        // container flags — leaving `container` set would send this straight
        // back into the container branch.
        auto slot = [&](const Value& v, PinType t, const std::string& tn)
        {
            StructField elem = f;
            elem.isArray = false; elem.container = HorizonCode::ContainerKind::None;
            elem.type = t; elem.typeName = tn;
            elem.defaultValue = v;
            elem.defaultValue.type = t;
            const std::string one = fieldInitializer(elem, nt, reg);
            // fieldInitializer yields " = <expr>"; strip the assignment.
            return one.size() > 3 ? one.substr(3) : std::string("{}");
        };
        if (f.kind() == HorizonCode::ContainerKind::Map)
        {
            const size_t n = std::min(f.defaultValue.keys.size(), f.defaultValue.items.size());
            if (n == 0) return {};
            std::string init = " = { ";
            for (size_t i = 0; i < n; ++i)
            {
                if (i) init += ", ";
                init += "{ " + slot(f.defaultValue.keys[i], f.keyType, f.keyTypeName) + ", " +
                        slot(f.defaultValue.items[i], f.type, f.typeName) + " }";
            }
            return init + " }";
        }
        ...
        {
            if (i) init += ", ";
            init += slot(f.defaultValue.items[i], f.type, f.typeName);
        }
        return init + " }";
    }
...
std::string generateCppTypesHeader()
{
    ...
    // (HeVec3 was emitted TWICE here, which is a redefinition — the generated
    // header did not compile at all for a project that used it.)
