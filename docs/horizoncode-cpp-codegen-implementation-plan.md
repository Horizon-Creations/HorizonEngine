...
| `HE::api` + machine-readable `ApiFn` registry (19 groups / ~140 fns; `id`, `category`, `isExec`, typed `params`/`results`, `cppCall`, Value-marshalling `invoke`, `displayName`) | `src/HE_Scene/include/HorizonScene/EngineApi.h` + `src/EngineApi.cpp` | complete; the single source the EngineCall node, Lua/Py dispatchers and codegen all read — codegen goes through `invoke` (§7.2), `cppCall` is staged for the §7.1 v2 direct-call migration and is not emitted yet |
