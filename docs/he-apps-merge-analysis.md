...
- ~~Tabelle mit Kopfzeile (B2b)~~ **erledigt am 07.09.2026**: eine Eigenschaft an `UIListView` (`Show Header` plus acht), kein neuer Elementtyp. Die Spaltentitel sind die NAMEN der Zellen in der Zeilenvorlage, aus dem Asset gelesen (eine Tabelle ohne Einträge hat trotzdem eine Kopfzeile), `Column Widths` als Verhältnis-String, Trenner ziehbar mit fester Summe, `OnHeaderClicked` mit dem Spaltenindex — sortiert wird beim Besitzer. Neu in der Basis: `UIScrollBarStyle::topInset` und `UIElement::childClipTopInset()`. Vorbedingung an die Vorlage: eine einzelne Horizontal Box, deren Zellen weder Slot Fill noch `Auto Size` auf x benutzen — sonst eine Warnung pro Vorlage und null Spalten. Nachtrag in `docs/he-apps-plan.md` §13.1a.
- ~~Akkordeon (B5)~~ **erledigt am 07.09.2026**: neuer Elementtyp `UIWidgetType::Accordion` nach §13.2. Kinder sind die Abschnitte, ihre Namen die Überschriften; ein zugeklappter Körper ist null hoch, also ist die Höhe die Summe der offenen und alles darunter rutscht nach — dafür eine eigene `accordionSlotRect`. `Expanded` ist eine Int-Bitmaske mit Deckel bei 32, im Panel als Reihe Kästchen mit den Abschnittsnamen; gelesen wird sie überall durch `normalizedMask`, sonst lässt `Allow Multiple` aus nach einem Set Property zwei Bits stehen. Scrollen kostete die drei bekannten Virtuals, Animation gibt es bewusst keine. Nachtrag in `docs/he-apps-plan.md` §13.2a.
- ~~Mehrere Fenster (A5)~~ **erledigt am 07.09.2026** nach §13.3, auf Software und Metal. `Instance::windowId` (Laufzeit, nichts davon in einer `.hwidget`), `extract`/`processPointer`/`setDisplayScale` je mit einer Fensternummer davor und einer Überladung ohne sie für Fenster 0. Grabs sind **fenstermodal** (`topGrabOf`): ein Dialog sperrt sein Fenster, das daneben macht weiter, und Escape schließt die oberste Ebene DES FENSTERS, in dem es gedrückt wurde. Skriptseite: `window.open/close/setTitle/setSize/show` über die Registry, also HorizonCode, Lua und Python gleichzeitig, plus `OnWindowClosed`. `Capabilities::supportsSecondaryWindows` sagt an EINER Stelle nein für GL/Vulkan/D3D, statt dass vier Backends ein leeres schwarzes Fenster aufmachen. **Kein `WindowState`-Struct** — was der Zuschnitt dafür aufzählte, gehört zur Hälfte dem einen Zeiger und der einen Tastatur; die Begründung und die drei offenen Punkte (nie auf echter Hardware gesehen, kein gemerkter Fokus pro Fenster, nicht app-modal) stehen in §13.3a. Datei-Watcher ist gebaut (`fs.watch`, `bfb3f28e`), aber die Zustellung läuft nur im Spiel-Host.
