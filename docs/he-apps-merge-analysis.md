...
- Akkordeon (B5) und mehrere Fenster (A5) — **weiterhin nicht gebaut**, aber seit dem 07.09.2026 je mit einem Umsetzungsplan hinterlegt: `docs/he-apps-plan.md` §13 (13.1 Tabelle, 13.2 Akkordeon, 13.3 mehrere Fenster), mit Entscheidungen, berührten Dateien, Tests und Größe. §13.3 nennt die drei Befunde, die A5 den Zuschnitt geben: die UI hängt am Weltschnappschuss, `RenderWindow` ruft niemand auf und ist auf GL nur ein `TODO`, und ein Zweitfenster wird heute still abgeräumt. Datei-Watcher ist gebaut (`fs.watch`, `bfb3f28e`), aber die Zustellung läuft nur im Spiel-Host.
